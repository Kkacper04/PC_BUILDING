...
def list_cpus(
    ...
    return query.order_by(CPU.id.asc()).offset(offset).limit(limit).all()
...
def list_gpus(
    ...
    return query.order_by(GPU.id.asc()).offset(offset).limit(limit).all()
...
def get_motherboards(
    ...
    return query.order_by(Motherboard.id.asc()).offset(offset).limit(limit).all()
...
def get_ram(
    ...
    return query.order_by(RAM.id.asc()).offset(offset).limit(limit).all()

@router.get("/ram/{item_id}", response_model=RAMResponse)
def get_ram_item(item_id: int, db: Session = Depends(get_db)):
...
def get_psus(
    ...
    return query.order_by(PSU.id.asc()).offset(offset).limit(limit).all()

@router.get("/psus/{item_id}", response_model=PSUResponse)
def get_psu_item(item_id: int, db: Session = Depends(get_db)):
...
def get_cases(
    ...
    query = db.query(Case)
    return query.order_by(Case.id.asc()).offset(offset).limit(limit).all()

@router.get("/cases/{item_id}", response_model=CaseResponse)
def get_case_item(item_id: int, db: Session = Depends(get_db)):
...
def get_storage(
    ...
    return query.order_by(Storage.id.asc()).offset(offset).limit(limit).all()
